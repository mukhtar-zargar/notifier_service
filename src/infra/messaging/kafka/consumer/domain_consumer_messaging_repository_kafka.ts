...
export class DomainConsumerMessagingRepositoryKafka implements IDomainConsumerMessagingRepository {
  ...
  async subscribe(
    ...
    const topics = subscriptionParametersList.map((plist) => plist.topic).filter((v, i, a) => a.indexOf(v) === i);
    ...
    await consumer.run({
      autoCommitInterval: consumerOptions?.autoCommitInterval || DefaultConsumerOptions.autoCommitInterval,
      autoCommitThreshold: consumerOptions?.autoCommitThreshold || DefaultConsumerOptions.autoCommitThreshold,
      eachMessage: async (payload: EachMessagePayload) => {
        ...
        const subscriptionPromises = subscriptionParametersList.map(async (subscriptionParameters) => {
          ...
          if (isSubscribed) {
            const decodedMessage = await this.decodeMessage(noAvroDecoding, payload);
            // Log
            // TODO: Namespace runInNewContext
  ...
  private async enqueueForProcessing(decodedMessage: any, subscriptionParameters: SubscriptionParameters, consumer: any) {
  ...
  private isHandlerSubscribed(subscriptionParameters: SubscriptionParameters, incomingMessageTopic: string, eventType: string): boolean {
    return (
